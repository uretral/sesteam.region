...
trait ModelTreeMenu
{
    ...
    public function setLinkColumn($column)
    {
        $this->titleColumn = $column;
    }
     ...
     *            dump($node);
    dump(Category::root($node[$this->getKeyName()]));
    //            $node[$this->linkColumn] = Category::root($node[$this->getKeyName()]);
     * @param array $tree
     * @param int   $parentId
     */
    public static function saveOrder($tree = [], $parentId = 0)
    {
        ...
        foreach ($tree as $branch) {
            ...
            $node->{$node->getLinkColumn()} = Category::root($node['id']);
            $node->save();
    ...
    protected static function boot()
    {
        ...
        static::saving(function (Model $branch) {
            ...
            if (Request::has('_order')) {
                ...
                static::treeMenu()->saveOrder($order);
